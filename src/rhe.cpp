...
void compute_se(MatrixXdr &y,MatrixXdr &se, double h2g, double h2e,double tr_k2 )
{
	cout<<"p: "<<p << "  n: "<<n<<endl; 
	MatrixXdr zb=y;
...
//		clock_t rest = clock(); 
	...
	double vg,ve; 
	for(int i=0; i<pheno_num; i++){
		cout<< "Variance Component estimation for phenotype "<<i+1<<" :"<<endl; 
		MatrixXdr b(2,1); 
		b(0,0) = yKy(i,i); 
		b(1,0) = yy(i,i); 
